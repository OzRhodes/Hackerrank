#circularArrayRotation.py


def circularArrayRotation(a, k, queries):
    # mod the number of rotations
    # then use the remainder as the
    # factor to add to the query to extract the index
    # a = []
    # k = rotation
    # queries = number of queries
    answers = []
    array_len = len(a)
    steps = k % array_len    # assumes we rotate more than once
    for query in range (len(queries)):
        index = queries[query]-steps
        if index < 0:
            index = array_len + index
        answer = a[index]
        answers.append(answer)
    return answers
    
a = [39356,87674,16667,54260,43958,70429,53682,6169,87496,66190,90286,4912,44792,65142,36183,43856,77633,38902,1407,88185,80399,72940,97555,23941,96271,49288,27021,32032,75662,69161,33581,15017,56835,66599,69277,17144,37027,39310,23312,24523,5499,13597,45786,66642,95090,98320,26849,72722,37221,28255,60906]
k = 52
queries = [47,10,12]
print (circularArrayRotation(a, k, queries))

72722

90286

44792